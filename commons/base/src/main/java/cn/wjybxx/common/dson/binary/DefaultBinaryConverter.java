...
import cn.wjybxx.common.dson.binary.codecs.ProtoUtils;
...
public class DefaultBinaryConverter implements BinaryConverter {
    ...
    private DefaultBinaryConverter(TypeIdRegistry typeIdRegistry, BinaryCodecRegistry codecRegistry, int recursionLimit) {
    ...
    public static DefaultBinaryConverter newInstance(final Set<String> protoBufPackages,
                                                     ...
                                                     final int recursionLimit) {
        final Set<Class<?>> allProtoBufClasses = ProtoUtils.scan(protoBufPackages);
        ...
        final Map<Class<?>, BinClassId> typeIdMap = allClasses.stream()
    ...
    public static DefaultBinaryConverter newInstance(final Set<Class<?>> allProtoBufClasses,
        ...
        for (Class<?> clazz : allProtoBufClasses) {
            BinClassId binClassId = CollectionUtils.checkGet(typeIdMap, clazz, "class");
            if (binClassId.getNamespace() <= 0) {
                throw new IllegalArgumentException("bad namespace " + binClassId.getNamespace());
            }
        }
        for (BinaryPojoCodecImpl<?> codecImpl : pojoCodecImplList) {
            BinClassId binClassId = CollectionUtils.checkGet(typeIdMap, codecImpl.getEncoderClass(), "class");
            if (binClassId.getNamespace() <= 0) {
                throw new IllegalArgumentException("bad namespace " + binClassId.getNamespace());
            }
        }
        ...
        final TypeIdRegistry typeIdRegistry = TypeIdRegistries.fromRegistries(TypeIdRegistries.fromTypeIdMap(typeIdMap),
        ...
        return new DefaultBinaryConverter(typeIdRegistry, codecRegistry, recursionLimit);
    }
