...
public final class DsonEnums {
    ...
    public static <T extends DsonEnum> void checkNumberDuplicate(List<T> values) {
        ...
        for (T t : values) {
            final int number = t.getNumber();
            if (!numberSet.add(number)) {
                final String msg = String.format("The number is duplicate, num: %d, enum: %s", number, t);
    ...
    public static <T extends DsonEnum> void checkNumberContinuity(List<T> values, @Nullable Integer baseNumber) {
        ...
        // 检查起始值
        if (baseNumber != null) {
            int firstNumber = values.get(0).getNumber();
        ...
        for (int index = 0; index < values.size() - 1; index++) {
            final int curNumber = values.get(index).getNumber();
            final int nextNumber = values.get(index + 1).getNumber();
