...
public class DefaultDsonBinReader implements DsonBinReader {
    ...
    public DsonValueSummary peekValueSummary() {
        ...
        DsonValueSummary header;
        switch (dsonType) {
            case STRING -> {
                header = new DsonValueSummary(dsonType, input.readUint32(), (byte) 0, null);
            }
            case EXT_INT32, EXT_INT64 -> {
                byte subType = input.readRawByte();
                header = new DsonValueSummary(dsonType, 0, subType, null);
            }
            case EXT_STRING -> {
                byte subType = input.readRawByte();
                header = new DsonValueSummary(dsonType, input.readUint32(), subType, null);
            }
            case BINARY -> {
                int length = input.readFixed32();
                byte subType = input.readRawByte();
                header = new DsonValueSummary(dsonType, length, subType, null);
            }
            case ARRAY, OBJECT -> {
                int length = input.readFixed32();
                BinClassId classId = readClassId();
                header = new DsonValueSummary(dsonType, length, (byte) 0, classId);
            }
            default -> {
                header = new DsonValueSummary(dsonType, 0, (byte) 0, null);
            }
        }
