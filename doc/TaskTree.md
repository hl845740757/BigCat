...
由于上一次的执尚未完全退出，因此现在**有"两个"状态A都在execute代码块**，我们称这种情况为重入、
...
我添加机制检测任务似乎正在执行，重入时如果正在执行则抛出错误。
...
**解决方案**：  
我们在每个Task上记录一个reentryId，当Task的生命周期发生变更时+1；用户在执行不确定代码前，先将reentryId保存下来，执行不确定代码后，
