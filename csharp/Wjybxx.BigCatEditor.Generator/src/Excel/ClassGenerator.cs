...
using ClassCfg = Wjybxx.BigCatEditor.Generator.Excel.ClassGeneratorCfg.ClassCfg;
using FieldCfg = Wjybxx.BigCatEditor.Generator.Excel.ClassGeneratorCfg.FieldCfg;
...
#nullable enable
    ...
    public void Execute() {
        ...
        foreach (string fileName in _fileNames) {
            ...
            if (dsFile == null) {
                throw new InvalidOperationException("ds file not fount: " + _fileNames);
            }
    ...
    private void Generate(DSNamedType namedType) {
        ...
        if (namedType.BaseType != null) {
            ClassName baseTypeName = ClassName.Get(_cfg.ns, namedType.BaseType.SimpleName);
        ...
        foreach (DSElement element in namedType.EnclosedElements) {
            ...
            string readMethodName = GeneratorUtil.GetReadMethodName(fieldTypeName);
            if (IsSsti(field)) {
                ...
                // List解码需要传入声明类型
                if (readMethodName == GeneratorUtil.METHOD_NAME_READ_OBJECT) {
                ...
                } else if (readMethodName == GeneratorUtil.METHOD_NAME_READ_OBJECT) {
                    // ReadObject需要传声明类型
    ...
    private static string? GetFieldDecodeProxy(ClassCfg? classCfg, DSField field) {
        if (classCfg == null) {
            return null;
        }
        return classCfg.fieldProxies.TryGetValue(field.SimpleName, out string? proxy) ? proxy : null;
    }

    private static bool IsSsti(DSField field) {
