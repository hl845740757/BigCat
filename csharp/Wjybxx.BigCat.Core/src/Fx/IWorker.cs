...
/// 4. 规划上不在同一个Worker的Module，在部署时不应该部署在同一个Worker，否则可能引发死锁等问题。
...
/// 事件循环更多的是负责与其它线程打交道。
/// </summary>
public interface IWorker : IDisruptorEventLoop<WorkerEvent>
{
    /// <summary>
    /// Worker绑的对象容器
    /// </summary>
...
#nullable disable
    ...
    /// 注：正常业务不应该通过该字段获取引用。
    /// </summary>
    public static IWorker CurrentWorker => FxUtils.CURRENT_WORKER.Value;

    /// <summary>
    /// 设置当前线程的Worker
    /// </summary>
    /// <param name="worker"></param>
    public static void SetCurrentWorker(IWorker worker) {
