...
{
public class VarRectField : MRectField, IVarField
{
    ...
    public void Bind(DataEditor editor, Variable variable) {
        _variable = variable;
        this.SetValueWithoutNotify(variable.rectValue);
    }
    ...
    private void OnValueChanged(ChangeEvent<Rect> evt) {
        if (_variable != null) {
            _variable.rectValue = evt.newValue;
    ...
    public void Refresh(bool rebuild = false) {
        if (_variable != null) {
            SetValueWithoutNotify(_variable.rectValue);
        }
    }
}
}