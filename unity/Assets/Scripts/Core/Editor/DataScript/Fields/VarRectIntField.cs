...
{
public class VarRectIntField : MRectIntField, IVarField
{
    private Variable _variable;

    public VarRectIntField() {
    ...
    public void Bind(DataEditor editor, Variable variable) {
        _variable = variable;
        this.SetValueWithoutNotify(variable.rectIntValue);
    }
    ...
    private void OnValueChanged(ChangeEvent<RectInt> evt) {
        if (_variable != null) {
            _variable.rectIntValue = evt.newValue;
    ...
    public void Refresh(bool rebuild = false) {
        if (_variable != null) {
            SetValueWithoutNotify(_variable.rectIntValue);
        }
    }
}
}