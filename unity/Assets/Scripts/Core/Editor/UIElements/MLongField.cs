...
public class MLongField : LongField, IField
{
    ...
    private long Clamp(long newValue) {
        if (hasMin && newValue < min) return min;
        if (hasMax && newValue > max) return max;
        return newValue;
    }

    public new class UxmlFactory : UxmlFactory<MLongField, UxmlTraits>
    {
    }
    ...
    public new class UxmlTraits : TextValueFieldTraits<long, UxmlLongAttributeDescription>
    {
        ...
        private readonly UxmlLongAttributeDescription maxAttribute = new()
        {
            name = "max",
            defaultValue = 0,
        };
