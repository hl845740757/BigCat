...
public class MDoubleField : DoubleField, IPrefixLabel
{
    ...
    private double Clamp(double newValue) {
        if (hasMin && newValue < min) return min;
        if (hasMax && newValue > max) return max;
        return newValue;
    }

    public new class UxmlFactory : UxmlFactory<MDoubleField, UxmlTraits>
    {
    }
    ...
    public new class UxmlTraits : DoubleField.UxmlTraits
    {
        ...
        private readonly UxmlDoubleAttributeDescription maxAttribute = new()
        {
            name = "max",
            defaultValue = 0,
        };
