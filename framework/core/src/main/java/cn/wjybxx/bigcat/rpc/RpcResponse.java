...
public final class RpcResponse extends RpcProtocol {
     ...
     */
    @DsonProperty(writeProxy = "writeResults", readProxy = "readResults")
...
//                ", results=" + results +
    ...
    // 1.自动处理延迟序列化问题
    // 2.避免多态写入List类型信息
    public void writeResults(DsonObjectWriter writer, String name) {
        if (results == null) {
        ...
        if (results instanceof byte[] bytes) {
            ...
            writer.writeStartArray(name, RpcRequest.getListTypeInfo(results));
    ...
    public void readResults(DsonObjectReader reader, String name) {
        ...
        while ((dsonType = reader.readDsonType()) != DsonType.END_OF_OBJECT) {
            if (dsonType == DsonType.HEADER) { // 用户可能写入了List的类型
                ...
                results.add(reader.readObject(null, TypeInfo.OBJECT));
            }
        }
        reader.readEndArray();
