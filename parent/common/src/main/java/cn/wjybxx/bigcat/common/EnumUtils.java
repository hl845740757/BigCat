...
public class EnumUtils {
    ...
    public static BitSet toBitSet(Enum<?>... values) {
    ...
    public static BitSet toBitSet(Collection<? extends Enum<?>> values) {
        final BitSet bitSet = new BitSet(64); // 可以自动扩容
        for (Enum<?> e : values) {
            bitSet.set(e.ordinal());
        }
     ...
     * @return unmodifiable
     */
    public static <T extends IndexableEnum> IndexableEnumMapper<T> mapping(T[] values, final boolean fastQuery) {
        if (values.length == 0) {
    ...
    private static class SingletonMapper<T extends IndexableEnum> implements IndexableEnumMapper<T> {
    ...
    private static class MapBasedMapper<T extends IndexableEnum> implements IndexableEnumMapper<T> {
        ...
        private MapBasedMapper(T[] values, Int2ObjectMap<T> mapping) {
            ...
            this.sortedValues = CollectionUtils.toImmutableList(this.values, Comparator.comparingInt(IndexableEnum::getNumber));
            this.isContinuous = isNumberContinuity(sortedValues, T::getNumber);
        }
