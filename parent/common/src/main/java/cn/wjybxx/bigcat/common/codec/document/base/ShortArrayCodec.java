...
public class ShortArrayCodec implements DocumentPojoCodecImpl<short[]> {
    ...
    @Override
    public void writeObject(short[] instance, DocumentWriter writer, TypeArgInfo<?> typeArgInfo) {
        for (short e : instance) {
            writer.writeShort(writer.nextElementName(), e);
        }
    }

    @Override
    public short[] readObject(DocumentReader reader, TypeArgInfo<?> typeArgInfo) {
        ShortArrayList result = new ShortArrayList();
        while (!reader.isAtEndOfObject()) {
            result.add(reader.readShort(reader.nextElementName()));
        }
