...
public abstract class AbstractBinaryPojoCodecImpl<T> implements BinaryPojoCodecImpl<T> {
    ...
    @Override
    public final T readObject(BinaryReader reader, TypeArgInfo<?> typeArgInfo) {
        final T instance;
     ...
     */
    public abstract void readFields(T instance, BinaryReader reader, TypeArgInfo<?> typeArgInfo);

    /**
     * 用于执行用户序列化完成的钩子方法
     */
    protected void afterDecode(T instance, BinaryReader reader, TypeArgInfo<?> typeArgInfo) {
