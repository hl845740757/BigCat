...
public class ShortArrayCodec implements BinaryPojoCodecImpl<short[]> {
    ...
    @Override
    public void writeObject(short[] instance, BinaryWriter writer, TypeArgInfo<?> typeArgInfo) {
    ...
    @Override
    public short[] readObject(BinaryReader reader, TypeArgInfo<?> typeArgInfo) {
        ShortArrayList result = new ShortArrayList();
        while (!reader.isAtEndOfObject()) {
            result.add(reader.readShort());
        }
